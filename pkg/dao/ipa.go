...
type IpaDAO interface {
	...
	// GetByName retrieves a single record by uniq key name from db.
	GetByName(ctx context.Context, name string) (*models.Ipa, error)
	// BatchGetByName retrieves multiple records by uniq key name from db.
	BatchGetByName(ctx context.Context, names []string) (map[string]*models.Ipa, error)
