...
func (d *IpaDAO) GetByName(ctx context.Context, name string) (*models.Ipa, error) {
	ipaObj := &models.Ipa{}

	sel := "*"
	query := fmt.Sprintf(
		"select %s from `ipa` where `name`=?", sel,
	)
	...
	if err != nil {
		if pkgErr.Cause(err) == sql.ErrNoRows {
			return nil, pkgErr.Wrapf(errors.ErrNotFound, "table=ipa, query=%s, args=name :%v", query, name)
		}
		return nil, pkgErr.Wrap(err, "dao: unable to select from ipa")
	}
...
func (d *IpaDAO) BatchGetByName(ctx context.Context, names []string) (map[string]*models.Ipa, error) {
	var exec boil.ContextExecutor
	txn := ctx.Value("txn")
	if txn == nil {
		exec = d.mysqlPool
	...
	datas, err := models.Ipas(models.IpaWhere.Name.IN(names)).All(ctx, exec)
	if err != nil {
		return nil, pkgErr.WithStack(err)
	}

	result := make(map[string]*models.Ipa)
	for _, c := range datas {
		result[c.Name] = c
	}
