...
var MemberPayOrderTableColumns = struct {
	ID        string
	MemberID  string
	Status    string
	Amount    string
	...
	ID:        "member_pay_order.id",
	MemberID:  "member_pay_order.member_id",
	Status:    "member_pay_order.status",
	Amount:    "member_pay_order.amount",
	BizExt:    "member_pay_order.biz_ext",
	CreatedAt: "member_pay_order.created_at",
	UpdatedAt: "member_pay_order.updated_at",
}
...
var (
	...
	memberPayOrderGeneratedColumns      = []string{}
)
...
func MemberPayOrders(mods ...qm.QueryMod) memberPayOrderQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`member_pay_order`.*"})
	}

	return memberPayOrderQuery{q}
}
...
// Exists checks if the MemberPayOrder row exists.
func (o *MemberPayOrder) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return MemberPayOrderExists(ctx, exec, o.ID)
}
