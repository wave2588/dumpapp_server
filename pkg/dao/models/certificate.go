...
func Certificates(mods ...qm.QueryMod) certificateQuery {
	mods = append(mods, qm.From("`certificate`"))
	return certificateQuery{NewQuery(mods...)}
}

// FindCertificate retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindCertificate(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Certificate, error) {
	...
	query := fmt.Sprintf(
		"select %s from `certificate` where `id`=?", sel,
	)
...
func (o *Certificate) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	o.ID = int64(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == certificateMapping["id"] {
		goto CacheNoHooks
	}
...
func CertificateExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `certificate` where `id`=? limit 1)"
