...
func (c *EmailController) SendRegisterEmail(ctx context.Context, title, content, receiveEmail string) error {
	fromEmail := config.DumpConfig.AppConfig.DumpAppRegisterFromEmail
	fromEmailPassword := config.DumpConfig.AppConfig.DumpAppRegisterFromEmailPassword
	m := gomail.NewMessage()
	m.SetHeader("From", fromEmail)  // 发件人
	m.SetHeader("To", receiveEmail) // 收件人
