...
func (d *{{$alias.UpSingular}}DAO) Delete(ctx context.Context, id int64) error {
	...
	d.InvalidateGet(ctx, id)
	return nil
}

const Get{{$alias.UpSingular}}CacheKey = "Get{{$alias.UpSingular}}"

func (d *{{$alias.UpSingular}}DAO) InvalidateGet(ctx context.Context, id int64) {
	d.cacheMCStore.MustEvict(ctx, func() string {
		return store.NewKey(Get{{$alias.UpSingular}}CacheKey, id)
	})
}
...
// BatchGetCache retrieves multiple records by primary key from cache.
func (d *{{$alias.UpSingular}}DAO) BatchGetCache(ctx context.Context, ids []int64) (map[int64]*models.{{$alias.UpSingular}}, error) {
    resultMap := make(map[int64]*models.{{$alias.UpSingular}})
	ttl := 24 * time.Hour
	err := d.cacheMCStore.GetMulti(ctx, ids,
		func(i interface{}) string {
			return store.NewKey(Get{{$alias.UpSingular}}CacheKey, i.(int64))
		},
		func(i interface{}) (interface{}, error) {
			ids := i.([]int64)
			return d.BatchGet(ctx, ids)
		}, &resultMap, &ttl)
	return resultMap, pkgErr.WithStack(err)
}

// BatchGetCache retrieves multiple records by primary key from db.
...
const Get{{$alias.UpSingular}}By{{$fnName}}CacheKey = "Get{{$alias.UpSingular}}By{{$fnName}}"

func (d *{{$alias.UpSingular}}DAO) InvalidateGetBy{{$fnName}}(ctx context.Context, {{$ukArgs}}) {
	d.cacheMCStore.MustEvict(ctx, func() string {
		return store.NewKey(Get{{$alias.UpSingular}}By{{$fnName}}CacheKey, {{$ukNameArgs}})
	})
}

// GetBy{{$fnName}} retrieves a single record by uniq key {{$ukNameArgs}} from cache.
func (d *{{$alias.UpSingular}}DAO) GetBy{{$fnName}}Cache(ctx context.Context, {{$ukArgs}}) (*models.{{$alias.UpSingular}}, error) {
	result := &models.{{$alias.UpSingular}}{}
    ttl := 24 * time.Hour
	err := d.cacheMCStore.Get(ctx,
		func() string {
            return store.NewKey(Get{{$alias.UpSingular}}By{{$fnName}}CacheKey, {{$ukNameArgs}})
		},
		func() (interface{}, error) {
			return d.GetBy{{$fnName}}(ctx, {{$ukNameArgs}})
		}, &result, &ttl)
	return result, err
}
...
// BatchGetBy{{$fnName}}Cache retrieves multiple records by uniq key {{$ukColName}} from cache.
func (d *{{$alias.UpSingular}}DAO) BatchGetBy{{$fnName}}Cache(ctx context.Context, {{$ukColName}}s []{{$ukColType}}) (map[{{$ukColType}}]*models.{{$alias.UpSingular}}, error) {
    resultMap := make(map[{{$ukColType}}]*models.{{$alias.UpSingular}})
	ttl := 24 * time.Hour
	err := d.cacheMCStore.GetMulti(ctx, {{$ukColName}}s,
		func(i interface{}) string {
			return store.NewKey(Get{{$alias.UpSingular}}CacheKey, i.({{$ukColType}}))
		},
		func(i interface{}) (interface{}, error) {
			ids := i.([]{{$ukColType}})
			return d.BatchGetBy{{$fnName}}(ctx, ids)
		}, &resultMap, &ttl)
	return resultMap, pkgErr.WithStack(err)
}
