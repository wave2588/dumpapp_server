...
func run() {
	...
	cerCount, err := impl.DefaultCertificateV2DAO.Count(ctx, filters)
	util.PanicIf(err)

	expendAmount := float64(cerCount * 25)
	totalAmount := cast.ToFloat64(int(amount) + cdKeyOrderAmount)
	...
	cerCountStr := fmt.Sprintf("证书：<font color=\"comment\">%d</font>\n", cerCount)
	...
	data := map[string]interface{}{
		...
		"markdown": map[string]interface{}{
			"content": contentStr +
				newMemberStr + paidMemberStr + downloadedStr + downloadedMemberStr + cerCountStr + orderCountStr + amountStr + cdKeyAmountStr + totalAmountStr + uvStr + pvStr + timeStr,
		},
	}
